-- Tighten free tier: 3 AI responses (down from 5), no free contract analyses.
-- Contract analysis is a Pro differentiator — free users see the upgrade prompt immediately.
...
-- Free users get 0 contract analyses — Pro only.
...
declare
  ...
  allowed boolean;
begin
  select plan, contracts_used, period_responses_used
  into current_plan, current_count, period_count
  from public.user_profiles
  where id = uid
  for update;
  ...
  if current_plan = 'pro' then
    ...
    -- free plan: no contract analyses
    allowed := false;
  end if;
